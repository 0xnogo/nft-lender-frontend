...
import { NFTLENDER_CONTRACT_ADDRESS, useDeposit } from '../../hooks/use-nftlender';
    ...
    <Container title="Deposit">
      <form className="flex flex-col gap-y-2">
        ...
        <input 
          ...
          className={inputStyle} />
        {isApproved && !isDeposited && <Button disabled={!deposit} text={isLoadingDeposit ? "Depositing..." : "Deposit"} onClickHandler={onDepositHandler} style="btn-primary" />}
        {!isApproved && !isDeposited && <Button disabled={!approve} text={isLoadingApprove ? 'Approving...' : 'Approve'} onClickHandler={onApproveHandler} style="btn-primary" />}
        {isDeposited && <Button disabled={isDeposited} text="Already deposited" onClickHandler={() => {}} style="btn-primary" />}
      </form>
    </Container>);
}
